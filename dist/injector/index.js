...
exports.formatForInjection = formatForInjection;
exports.formatAsJSON = formatAsJSON;
exports.formatSummary = formatSummary;
exports.formatDistillNotice = formatDistillNotice;
/**
 ...
 */
function formatForInjection(memories) {
 ...
 */
function formatAsJSON(memories) {
    return JSON.stringify(memories.map(({ memory, score }) => ({
        date: memory.date,
 ...
 */
function formatCompactSummary(memories) {
    const lines = [];
 ...
 */
function formatSummary(memories, branch) {
    if (memories.length === 0) {
    ...
    for (const { memory, score, breakdown } of memories) {
        ...
        const signals = [];
        if (breakdown.error_match && breakdown.error_match > 0)
            signals.push(`error:${Math.round(breakdown.error_match)}%`);
        if (breakdown.file_overlap > 0)
            signals.push(`files:${Math.round(breakdown.file_overlap)}%`);
 ...
 */
function formatDistillNotice(record) {
