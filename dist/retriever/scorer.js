...
function computeFileOverlap(memoryFiles, contextFiles) {
    ...
    const isRelevantFile = (f) => !ignorePatterns.some((p) => f.toLowerCase().includes(p));
    const memoryNames = new Set(memoryFiles.filter(isRelevantFile).map((f) => f.split("/").pop().toLowerCase()));
